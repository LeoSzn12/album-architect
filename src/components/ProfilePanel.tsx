...
export const ProfilePanel: React.FC<ProfilePanelProps> = ({ displayName: initialName = 'New curator', initialBio = 'Building a point of view, one track at a time.', stats = {}, favoriteCount = 0, onProfileChange, onBackToGame }) => {
  ...
  return <section aria-labelledby="profile-panel-title" className="w-full max-w-4xl rounded-3xl border border-purple-500/30 bg-gray-900/90 p-6 shadow-2xl backdrop-blur-md sm:p-8">
    {onBackToGame && (
      <div className="mb-5 flex items-center justify-between pb-4 border-b border-gray-800">
        ...
        <span className="text-xs text-gray-500 font-semibold">Executive Curator ID</span>
      </div>
    )}
    <div className="mb-7 flex items-start justify-between border-b border-gray-800 pb-6"><div className="flex gap-3"><div className="flex h-12 w-12 items-center justify-center rounded-2xl bg-gradient-to-br from-cyan-500 to-purple-600 shadow-lg"><UserRound className="h-6 w-6 text-white" aria-hidden="true" /></div><div><p className="text-[10px] font-black uppercase tracking-[0.22em] text-cyan-400">Curator profile</p><h2 id="profile-panel-title" className="text-2xl font-black text-white">Your A&R card</h2><p className="mt-1 text-sm text-gray-400">A compact read on your taste and draft history.</p></div></div><button type="button" onClick={() => editing ? save() : setEditing(true)} className="flex items-center gap-1.5 rounded-lg border border-gray-700 bg-gray-950 px-3 py-2 text-xs font-black text-gray-300 hover:border-purple-500 hover:text-white">{editing ? <Check className="h-3.5 w-3.5" aria-hidden="true" /> : <Pencil className="h-3.5 w-3.5" aria-hidden="true" />}{editing ? 'Save' : 'Edit'}</button></div>
    <div className="grid gap-7 lg:grid-cols-[1fr_1.25fr]"><div className="rounded-2xl border border-gray-800 bg-gray-950/70 p-5">{editing ? <div className="space-y-4"><label className="block text-xs font-bold text-gray-400">Display name<input value={displayName} onChange={(event) => setDisplayName(event.target.value)} className="mt-1 w-full rounded-lg border border-gray-700 bg-gray-900 px-3 py-2 text-sm text-white outline-none focus:border-purple-400" /></label><label className="block text-xs font-bold text-gray-400">Bio<textarea value={bio} onChange={(event) => setBio(event.target.value)} rows={3} className="mt-1 w-full resize-none rounded-lg border border-gray-700 bg-gray-900 px-3 py-2 text-sm text-white outline-none focus:border-purple-400" /></label></div> : <><h3 className="text-xl font-black text-white">{displayName}</h3><p className="mt-2 text-sm leading-6 text-gray-400">{bio}</p></>}<div className="mt-5 flex flex-wrap gap-2"><span className="rounded-lg border border-pink-500/30 bg-pink-950/40 px-2.5 py-1.5 text-[10px] font-black uppercase text-pink-300"><Heart className="mr-1 inline h-3 w-3" aria-hidden="true" />{favoriteCount} favorites</span><span className="rounded-lg border border-purple-500/30 bg-purple-950/40 px-2.5 py-1.5 text-[10px] font-black uppercase text-purple-300"><Disc3 className="mr-1 inline h-3 w-3" aria-hidden="true" />{profileStats.favoriteGenre}</span></div><div className="mt-5 rounded-xl border border-cyan-500/20 bg-cyan-950/20 p-4"><p className="text-[10px] font-black uppercase tracking-wider text-cyan-300">Account</p>{!authState ? <p className="mt-1 text-xs text-gray-500">Checking sign-in status…</p> : !authState.configured ? <p className="mt-1 text-xs leading-5 text-gray-400">Guest demo mode is active. Configure Supabase to save builds across devices.</p> : authState.authenticated ? <><div className="mt-2 flex items-center justify-between gap-3"><p className="truncate text-xs text-gray-300">Connected{authState.user?.email ? ` · ${authState.user.email}` : ''}</p><button type="button" disabled={authBusy || deleteBusy} onClick={signOut} className="rounded-lg border border-gray-700 px-2.5 py-1.5 text-[10px] font-black uppercase text-gray-300 hover:border-cyan-400 hover:text-white">{authBusy ? 'Signing out…' : 'Sign out'}</button></div><button type="button" disabled={authBusy || deleteBusy} onClick={deleteAccount} className="mt-3 rounded-lg border border-red-900/80 px-2.5 py-1.5 text-[10px] font-black uppercase text-red-300 hover:border-red-500 hover:text-red-200">{deleteBusy ? 'Deleting account…' : 'Delete account'}</button>{deleteError ? <p role="alert" className="mt-2 text-xs text-red-300">{deleteError}</p> : null}</> : <div className="mt-2 flex flex-wrap gap-2"><button type="button" onClick={() => signIn('google')} className="rounded-lg bg-white px-2.5 py-1.5 text-[10px] font-black uppercase text-gray-900 hover:bg-cyan-100">Sign in with Google</button><button type="button" onClick={() => signIn('github')} className="rounded-lg border border-gray-700 px-2.5 py-1.5 text-[10px] font-black uppercase text-gray-300 hover:border-cyan-400 hover:text-white">GitHub</button></div>}</div></div>
