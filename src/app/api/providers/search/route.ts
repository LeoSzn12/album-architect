...
export async function GET(request: NextRequest) {
  const limiter = rateLimit(requestRateLimitKey(request, 'provider-search'), { limit: 30, windowMs: 60_000 });
  if (!limiter.allowed) return NextResponse.json({ error: 'Search is temporarily rate limited. Please retry shortly.' }, { status: 429, headers: rateLimitHeaders(limiter) });
  ...
  return result.ok
    ? NextResponse.json({ provider: providerId, songs: result.data }, { headers: rateLimitHeaders(limiter) })
    : NextResponse.json({ provider: providerId, error: result.error }, { status: result.error.code === 'invalid-request' ? 400 : 503, headers: rateLimitHeaders(limiter) });
}
