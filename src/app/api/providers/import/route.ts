...
export async function POST(request: NextRequest) {
  ...
  if (result.ok) {
    ...
    return NextResponse.json({ provider: body.provider, playlist: result.data, persistedCount: persisted.persistedCount });
  }
  return NextResponse.json({ provider: body.provider, error: result.error }, { status: result.error.code === 'invalid-request' ? 400 : 503 });
}
